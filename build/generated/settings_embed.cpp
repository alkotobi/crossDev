  ...
  (function(){
    ...
    function saveOptions(){
      ...
      window.CrossDev.invoke('writeOptions',{options:options}).then(function(r){
        saving=false;
        if(r&&r.success){
          show($id('saveSuccess'));$id('saveBtn').textContent='Save';setTimeout(function(){hide($id('saveSuccess'));},3000);
          console.log('[Settings] Options saved successfully, reloading main window...');
          window.CrossDev.invoke('reloadMainWindow',{}).then(function(rr){
            console.log('[Settings] Main window reload response:', rr);
          }).catch(function(e){
            console.error('[Settings] Failed to reload main window:', e);
          });
        }
        else{$id('error').textContent=r&&r.error?r.error:'Failed to save';show($id('error'));console.error('[Settings] Save failed:',r&&r.error);}
      }).catch(function(e){
        saving=false;$id('error').textContent=e&&e.message?e.message:String(e);show($id('error'));console.error('[Settings] Save request failed:',e);
      });
      $id('saveBtn').textContent='Saving...';
    }
    function browseFile(filterKey,path,inputEl){
      if(!hasCrossDev()||!inputEl){return;}
      var titles={html:'Select HTML file',js:'Select preload script'};
      var filters={html:'HTML Files (*.html)|*.html|All Files (*.*)|*.*',js:'JavaScript (*.js)|*.js|All Files (*.*)|*.*'};
      var title=titles[filterKey]||'Choose file';
      var filter=filters[filterKey]||'All Files (*.*)|*.*';
      window.CrossDev.invoke('openFileDialog',{title)SEmB" R"SEmB(:title,filter:filter}).then(function(r){
        if(r&&r.success&&r.filePath){inputEl.value=r.filePath;}
      }).catch(function(){});
    }
